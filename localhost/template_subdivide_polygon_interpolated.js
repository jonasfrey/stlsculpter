...
return { n_x, n_y, n_z : 0 };
    ...
    function f_a_o_p(
        ...
        let a_o = new Array(n_corners).fill(0).map((v, n_idx) => {
            ...
            let n_sinbig = Math.sin(n_it_nor_corner*n_pc*n_tau*.5);
            n_amp += Math.sin(n_it_layer_nor*0.93*n_tau)*n_radius_base*0.5;
            n_amp += Math.sin(n_it_nor_corner*n_tau*n_pc*10)*n_sinbig*2.;
            let n_rad_offset = n_it_layer_nor*n_tau/n_pc;
            
            let o_trn =  f_o_p_on_polygon(n_it_nor_corner,n_pc, n_amp, n_rad_offset)
            let n_r1 = f_n_dist(o_trn);
            let n_r2 = n_radius_base;
            let nt = 0.9;
            let nr = nt*n_r1+(1.-nt)*n_r2;
            let o_trn_circle = {
                n_x: Math.sin(n_it_nor_corner*n_tau)*nr,
                n_y: Math.cos(n_it_nor_corner*n_tau)*nr,
            }
