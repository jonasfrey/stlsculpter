function() {
    ...
    function f_a_o_p(
        ...
        let a_o = new Array(n_corners).fill(0).map((v, n_idx) => {
            ...
            // https://www.desmos.com/calculator/4jnzpmcxpg
            let a = Math.sin(nx*n_tau*.59)*.9;
            let b = Math.sin(nx*n_tau*4+(n_tau/2))*.2;
            let d = -1*Math.abs(nx-0.85)+0.1;
            let c = Math.max(a,b);
            let f = Math.max(c,d);
            n_amp=f*n_radius_base;
            let n_rad_offset = 0;
