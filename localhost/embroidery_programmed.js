...
function(){
    ...
    for(let n_it_x = 0; n_it_x < n_pixels_x; n_it_x++){
        for(let n_it_y = 0; n_it_y < n_pixels_y; n_it_y++){
            ...
            let n_dist_center = Math.sqrt(
                Math.pow(o_mesh_pixel.position.x, 2) + 
                Math.pow(o_mesh_pixel.position.y, 2)
            );
            let n_dist_center_normalized = n_dist_center / (n_pixels_x * (n_scl_x_pixel+n_scl_margin_pixel_x) / 2);
            let n_dcenter_sine = Math.sin(n_dist_center_normalized * 1 * n_tau);
            if(n_dcenter_sine < 0){
                continue
            }
