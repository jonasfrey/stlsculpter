function() {
    ...
    let a_o_random = new Array(n_its_randompoint).fill(o).map((v, n_idx) => {
        ...
        let o_mesh_sphere_geometry = new THREE.SphereGeometry(n_radius, 32, 16);
        let o_mesh_sphere_material = new THREE.MeshBasicMaterial({ 
            color: 0xffffff,  // wireframe color
            wireframe: true   // enable wireframe rendering
        });
        let o_mesh_sphere = new THREE.Mesh(o_mesh_sphere_geometry, o_mesh_sphere_material);
        a_o_mesh.push(o_mesh_sphere);
        debugger
    ...
    a_o_mesh.push(
        ...a_o_geometry.map(o => { return f_o_shaded_mesh(o) })
    )
