async function() {
    ...
    let n_its_layer = 4
    let n_thick_base = 0.2
    let n_thick_per_layer = 2
    ...
    let n_its = 30;
    let n_radius1 = 15;
    for(let n_it = 1; n_it < n_its; n_it+=1){
        let n_it_nor = n_it / n_its;
        let n_ang1 = n_it_nor
        let n_ang2 = (n_it_nor + 0.5)%1;
        let n_it_nor2 = Math.abs(n_it_nor-.5)*2.;
        let np = Math.pow(n_it_nor2,10);
        let n_radius2 = n_radius1 + np*10.;
        let o_p1 = {
            x: 0,
            y: n_radius2
        }
    ...
    o_mesh_ring.position.y = n_radius1+n_rad_ring;
