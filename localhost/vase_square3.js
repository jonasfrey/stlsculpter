function() {
    ...
    function f_a_o_p(
        ...
        let a_o = new Array(n_corners).fill(0).map((v, n_idx) => {
            ...
            let n_rad_offset = 0;
            let n_its = 12.;
            let ni2 = n_it_nor_corner*n_its;
            let ni2f = ni2%1;
            let off = (parseInt(ni2-.5)%2)*(n_tau/2.);
            let nsin1 = (Math.sin(n_it_layer_nor*3.*n_tau+off)*.5+0.5)
            let i2 = +.2+nsin1*0.4;
            if(Math.abs(ni2f-.5)*2. > i2){
                n_amp+=nsin1*3.;
            }
            n_amp+= Math.sin(n_it_layer_nor*0.96*n_tau)*n_radius_base*0.4;
